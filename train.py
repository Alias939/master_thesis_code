...
import scenario_dataset_creator
import scenario_dataset_creator_xml
import scenario_validation_creator
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained('clips/mfaq',
                                              additional_special_tokens = ["<Q>", "<A>", "<link>"])
    ...
    # #########   SCENARIO SPECIFIC FINETUNING
    # train_datasets = [scenario_dataset_creator_xml.create_scenario_dataset_xml()]
    # #########

    #########   TESTING/TRAINING FINETUNING
    train_datasets = [scenario_existing_dataset_creator.create_existing_scenario_dataset()]
    #########
    ...
    def collate_fn(batch):
        ...
        for item in batch:
            questions.append(item['question'] if data_args.no_special_token else f"{item['question']}")

            answers.append(item['answer'] if data_args.no_special_token else f"{item['answer']}")
    ...
    if training_args.do_train:
        ...
        scenario_validation_creator.validate_scenario(model,tokenizer)
