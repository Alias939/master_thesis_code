...
class MonolingualDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        while True:
            ...
            for pair in page:   #################THIS ONE
                #print(page["qa_pairs"])
                #print(pair['id'], ' ',pair["page_id"])

                #print(pair)
                #pair["page_id"] = pair['id']

                yield pair["qa_pairs"]             #############THIS ONE
            #yield page['qa_pairs']
...
class MonolingualSingleDataset(torch.utils.data.IterableDataset):
    def __init__(self, dataset, *, batch_size, seed = 42, epoch = 0, single_domain = False):
        self.dataset = dataset
        self.batch_size = batch_size
        self.seed = seed
...
class IterableSingleDataset(torch.utils.data.IterableDataset):
    def __init__(self, datasets, languages, *, probabilities = None, batch_size = 10, seed = 42, single_domain = False, alpha = 0.3):
        self.datasets = MonolingualDataset(datasets, batch_size=batch_size, single_domain=single_domain)
        self.batch_size = batch_size
        self.seed = seed
        self.alpha = alpha
